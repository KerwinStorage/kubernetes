...
| k8s-node01   | 192.168.80.49  | **etcd**, kubelet, kube-proxy, docker, haproxy, keepalived   |
| vip          | 192.168.80.100 |                                                              |

**注释：这里前期先部署三台机器：`k8s-master01`、`k8s-node01`、`k8s-node02`，后期将`k8s-master02`、`k8s-master02`和`vip`高可用地址加入进去。**
...
> docker在默认情况下使用Cgroup Driver 为cgroupfs，而k8s推荐使用systemd来代替cgroupfs。
...
kubernetes地址：https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md
...
### 4.5.6 创建ServiceAccount Key ——secret
...
![img](https://img2023.cnblogs.com/blog/1740081/202212/1740081-20221218183912942-1185065342.png)
...
### 8.3.3 所有k8s节点添加kube-proxy的配置和service文件
...
nginx-deployment-78c4d49446-jmms9   1/1     Running   0          90s
```
...
| **角色**     | **IP**        | **组件**                                                     |
| ------------ | ------------- | ------------------------------------------------------------ |
| k8s-master02 | 192.168.80.48 | kube-apiserver kube-controller-manager kubectl kubelet kube-proxy kube-scheduler docker |
| k8s-master03 | 192.168.80.49 | kube-apiserver kube-controller-manager kubectl kubelet kube-proxy kube-scheduler docker |

## 10.1 系统初始化

注：请回到 **[1. 环境准备](https://www.cnblogs.com/-k8s/p/17229593.html#T1)**、**[2.k8s基本组件安装](https://www.cnblogs.com/-k8s/p/17229593.html#T2)** 执行一遍。
...
主要规划：master01、master02 安装 keepalived 服务，虚拟vip：`192.168.80.100` 端口为`8443`，此前签发证书中已经加入了`192.168.80.100`地址。haproxy 是 k8s-master01和 k8s-master02 安装。
